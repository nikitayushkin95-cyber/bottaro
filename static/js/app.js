...
function initializeCards() {
    ...
    cards.forEach((cardElement, index) => {
        ...
        // Устанавливаем рубашку карты (всегда закрыта)
        const cardImage = cardElement.querySelector('.card-image');
        if (cardImage) {
            cardImage.src = 'static/cards/card_back.png';
            cardImage.alt = 'Закрытая карта';
            // Сохраняем путь к изображению карты для последующего открытия
            cardElement.setAttribute('data-card-image', `static/cards/images/${cardSlug}.jpg`);
        }
...
function updateSelectedCardsList() {
    // Не показываем список выбранных карт до нажатия "Продолжить"
    // Это обеспечивает полностью слепой выбор
    return;
}
...
function revealSelectedCards() {
    ...
    const continueBtn = document.getElementById('continueBtn');
    continueBtn.disabled = true;
    continueBtn.textContent = 'Открываем карты...';
    
    // Показываем блок с выбранными картами
    const selectedCardsBlock = document.getElementById('selectedCards');
    selectedCardsBlock.style.display = 'block';
    
    // Заполняем список выбранных карт
    ...
    // Анимация открытия карт по очереди
    selectedCardElements.forEach((cardElement, index) => {
        setTimeout(() => {
            // Получаем путь к изображению карты
            const cardImagePath = cardElement.getAttribute('data-card-image');
            const cardImage = cardElement.querySelector('.card-image');
            
            // Добавляем класс для анимации
            cardElement.classList.add('revealing');
            
            // Через небольшую задержку меняем изображение
            setTimeout(() => {
                cardImage.src = cardImagePath;
                cardImage.alt = cardElement.getAttribute('data-name');
                cardElement.classList.add('revealed');
                cardElement.classList.remove('revealing');
            }, 300);
            
        }, index * 500); // Задержка между картами
    });
    
    // После открытия всех карт отправляем данные
    setTimeout(() => {
        sendDataToBot();
    }, selectedCardElements.length * 500 + 1000);
}
